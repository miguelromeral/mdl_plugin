...
if ($ADMIN->fulltree) {
    ...
    $options = array();
        $options[0] = get_string('no', 'league');
        $options[1] = get_string('yes', 'league');
        
    
    $settings->add(new admin_setting_configselect('league_many_attempts',
            get_string('many_attempts', 'league'),
            get_string('many_attempts_desc', 'league'),
            1, $options));
